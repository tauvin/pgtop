...
- [ ] **TLS-поддержка** через `tokio-postgres-rustls` — без неё RDS / Cloud SQL / Heroku / Supabase / любой managed-DB не подключается. Параметр `sslmode` в DSN: `disable` / `require` / `verify-ca` / `verify-full`. По умолчанию `prefer` (как у `psql`).
- [ ] **Тесты на чистую логику** (не требуют БД): `Filter::matches`, `Sort::compare_backends`, форматтеры из `views/*` (duration, query truncation, lock object resolution). Цель — 30+ unit-тестов, чтобы будущие рефакторы не ломали поведение незаметно.
- [ ] **Snapshot-тесты UI** через `insta` + `ratatui::TestBackend`: рендерим Activity / Locks / Detail / ConfirmTerminate в фиксированную область, сверяем с эталонным `.snap`-файлом. Ловит регрессии в layout/colour-coding.
- [ ] **MSRV CI job** — отдельная джоба в `ci.yml`, билдит на `rustc 1.88`, чтобы не отъехало случайно (был инцидент с 0.1.0).
- [ ] **Demo-GIF** — vhs `.tape`-файл, при следующей записи автор запускает `vhs demo.tape`, кладёт в `docs/demo.gif`, раскомменчивает в README.
...
- [ ] **`pg_stat_database` tab (`5`)** — IO/deadlocks/conflicts/temp_bytes/blks_hit/blks_read per database. Polling 5s. Сортировка по любому столбцу.
- [ ] **`pg_stat_user_tables` tab (`6`)** — per-table: n_live_tup, n_dead_tup, последний vacuum/analyze (autovacuum vs ручной), seq_scan vs idx_scan ratio. Главный экран DBA.
- [ ] **Wait-events histogram tab (`7`)** — агрегат за последние 60 секунд: `wait_event_type` → `wait_event` → count. Из ring-буфера активити-снапшотов, sampling-based (не EBPF).
- [ ] **EXPLAIN-popup на Activity** — выбрать строку, нажать `e` → popup с `EXPLAIN (FORMAT TEXT) <query>` (без `ANALYZE` — безопасно). Гард по `read_only`-профилю.
- [ ] **Долгие-запросы alert** — query_start > N секунд (configurable per-profile) → подсветка строки + visual bell в footer.
