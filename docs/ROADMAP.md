...
- [x] Опционально: homebrew tap — реализовано в фазе 12 (`dist-workspace.toml` с `installers = ["shell", "homebrew"]` + `tap = "tauvin/homebrew-pgtop"`).
...
- [x] **Snapshot-тесты UI** через `insta` + `ratatui::TestBackend` — реализовано в фазе 13 #2 после time-injection (#1). 7 снапшотов на 120×24 покрывают Activity, Locks, Databases, Tables, Detail и ConfirmTerminate.
...
8. [x] **`Tab` / `SortBy` через `strum`** — `EnumIter`/`EnumString`/`IntoStaticStr` derive'ы заменяют ручные `all()/from_index()/from_id()/from_label()` (используется `Tab::iter()` + serialize-атрибуты). `index()` теперь через `iter().position()`. Tab::label() остался ручным match'ем (пробел в "Top Queries" мешает derive). 3 round-trip теста.
...
- [x] **Мелкие гнильцы**: `next_tab` теперь через `Tab::cycle_next()` (`iter().cycle().skip_while`, без `unwrap`); panic hook обёрнут в `std::sync::Once` (idempotent install); удалены stale `_to_u64`/`_force_action_command_use` хелперы, мёртвое поле `ActionResult.at`, мёртвое поле `ConnectionState.name`; сняты `#[allow(dead_code)]` с `App::connection_mut`/`App::set_active`/`ConnectionState` (на `Replica` оставлен с комментарием — модель ≠ render subset).
